...
- **`parameters/`**  
  Contains the saved parameters obtained from the second-model fitting procedure. The RDS file names have this form: `parameters_trials_notfull_X_phiY.rds`. `X` is replaced by: `p1`, `p3`, `p5` or `p7` depending on the number of signals used in the model. `Y` is replaced by ` `, `_500` or `_250` depending on the sampling frequency used to estimate these parameters.
