...
Computation of the residuals and Shapiro-Wilk test.
...
for (trial_n in seq_along(trials)) {
  ...
  for (ch in channels_index) {
    ...
    for (m in models) {
      ...
      y <- downsample(eeg_data[, idx[1:p]], m$ds)
...
Q-Q plot for p = 1 in function of sampling frequency (fs). Inside the code, you must choose the fs that you want to plot. The example is given for fs = 500 Hz.

```{r}
# Select the fs to be plotted (here 500 Hz)
residuals_plot <- residuals_df %>%
  dplyr::filter(fs == "500 Hz")
...
p1_500 <- ggplot(residuals_plot, aes(sample = residual)) +
  stat_qq(shape = 1) +
  geom_qq(
